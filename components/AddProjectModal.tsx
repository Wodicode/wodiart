...
export function AddProjectModal({ onClose, onAdd }: AddProjectModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md rounded-lg bg-offwhite p-6 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="font-sora text-lg font-semibold text-charcoal">Add project</h2>
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <label className="block">
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-charcoal/50">
              Project name
            </span>
            <input
              type="text"
